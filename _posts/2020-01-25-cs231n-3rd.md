...
![Multiclass SVM loss](/assets/images/cs231n_3_hinge.png)
...
![SVM example](/assets/images/cs231n_3_ex.png)
...
Hinge loss는 loss를 계산할 때 최종 output $s_i=f(x_i,W)$를 바로 사용했지만, cross entropy는 0과 1사이의 값으로 바꾼 다음 계산하는 것이 차이점이다. 또한, hinge loss는 정답 class의 값이 가장 높으면 0으로 loss를 부여하고 그 값이 어떻든 신경 쓰지 않았다. 하지만, cross entropy는 **얼마나 정확하게** 분류했는지까지 수치화한다. 한 가지 더 짚고 넘어가자면 $y=logx$의 그래프는 다음과 같은데 $x$값이 1과 가까울수록 0에 가까워지고, 0과 가까울수록 $-\infty$에 가까워진다. 따라서 정답 클래스의 softmax 값이 클수록(1에 가까울수록) loss는 0에 가까워진다. 전체 식에서 $-$부호가 있는 것은 정답 클래스의 값이 낮아질수록 loss가 커지게 하기 위함이다.

![y=logx](/assets/images/logx.png)
...
Training data로부터 손실함수를 최소화하는 parameter $W$를 찾는 것은 좋은데, 우리의 최종 목표는 새로운 데이터(test data)도 잘 맞추는 분류기를 만드는 것이다. 따라서 training data에 너무 최적화된(손실함수를 과도하게 최소화한) $W$는 새로운 데이터에 잘 맞지 않을 가능성이 높다. 이를 **overfitting**(과적합)되었다고 한다. overfitting을 피하기 위한 방법으로 **regularization**(규제화)가 있다.

![regularization](/assets/images/regularization.png)

파란 원은 train data이고, 초록색 사각형은 test data이다. 파란 선을 보면 train data에 아주 잘 맞는 것을 볼 수 있지만(overfitting), test data에는 하나도 맞지 않는다. 따라서 우리는 초록색 선과 같이 train data에 완벽하게 맞지는 않지만, 새로운 데이터에 대해서도 어느정도 맞는 결과를 원한다. $\lambda R(W)$항이 그 역할을 한다. $\lambda R(W)$가 더해진 것이 왜 규제화냐면 전체 loss인 $L(W)$를 최소화하기 위해 $\lambda R(W)$가 커지는 것도 극복하라는 것이다. L1, L2 regularization 등의 규제화 방법이 있다.
