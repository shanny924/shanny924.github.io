...
앞이 보이지 않는 깜깜한 산에서 내려가는 방법을 생각해보자. 아마도 발을 이리저리 디디면서 아래쪽이라고 생각되는 방향으로 한 발짝씩 나아갈 것이다. Gradient Descent(GD, 경사 하강법)은 이러한 아이디어에 기반한 최솟값을 찾는 방법이다. 식은 다음과 같다.
...
4. Loss function를 최소화하는 $W$로 수렴할때까지 1~2번 과정을 반복한다.

<center>

![Gradient Descent](/public/images/gradient_multidimentional.gif)

</center>

언뜻보면 굉장히 간단하고 문제가 없어보이지만 몇가지 문제가 있다.

> 1. local minima에 빠질 수 있다.

위의 그림처럼 최종적으로 도달하는 곳이 loss function이 최솟값을 갖는 곳이면 문제가 없지만 아래와 같이 빨간 점에서 시작한다면 local minima에 빠져버리게 된다.

![Local Minima](/public/images/gradient-local-minima.png)

> 2. 속도가 굉장히 느려질 수 있다.

10만개의 training data가 있다고 하자. 그러면 2번 과정에서 loss와 gradient를 구할때 10만개의 데이터를 전부 사용해야 한다. 데이터가 100만, 1000만, 그 이상으로 커진다면? 한 번 학습할 때 너무 많은 시간이 소요된다.
...
딥러닝에서 학습과정은 데이터를 입력의 출력까지의 forward pass(순방향 과정)와 출력으로부터 $W$를 업데이트하는 backward pass(역방향 과정)로 구분할 수 있다. 전체 데이터 셋에 대해 순방향+역방향 과정이 전부 완료되면 1 epoch이 완료되었다고 한다.
...
GD, SGD, MGD 외에도 여러 optimizer가 있다. 크게 adaptive 계열과 momentum 계열이 있는데 최근에는 두 가지가 혼합된 Adam이 주로 사용된다.
