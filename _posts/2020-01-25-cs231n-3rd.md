...
손실 함수 $L(W)$에 $R(W)$(regularization term)를 더함으로써 과적합을 피할 수 있다. 직관적으로 설명하자면, $L(W)$를 최소화하 되 $R(W)$가 커지는 것을 극복하면서 최소화하라는 의미이다. 정규화의 종류로는
...
L1 정규화 항을 추가하면 변수 선택의 효과를 얻을 수 있다. 이는 특정한 $W_{k,l}$가 0이 되기 때문이다. 따라서 $W$가 sparse해지면서 초록색 선과 같은 단순한 결과가 얻게 된다.
...
손실 함수를 최소화하면 할수록 왼쪽 항의 값은 작아지지만, 모델이 복잡해지면서(파란 선의 모습) 오른쪽 항의 값이 커지게 된다. 따라서 특정 $W_{k,l}$의 값을 줄이는 과정이 필요하다. 또한 L1 정규화 항은 [laplace prior](https://en.wikipedia.org/wiki/Laplace_distribution)( $laplace(0, \frac{1}{\lambda})$ )를 사용하는 MAP inference와 동치이다.

![laplace dist'](/assets/images/laplace.png)
...
![Normal dist'](/assets/images/normal.png)
